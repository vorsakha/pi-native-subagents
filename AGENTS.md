...
- Never update an existing Pi package by installing this working checkout as a local path. Local and Git sources have different package identities and will load the extension twice. After pushing a release, update the existing Git source at the exact pushed commit; keep daily-driver Pi settings synchronized through the applicable configuration workflow.
