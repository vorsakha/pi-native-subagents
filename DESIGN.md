...
`harness: "auto"` is opt-in. It may select by explicit capability requirements or, when requirements are omitted, by healthy/authenticated catalog preflight. Runtime code does not classify arbitrary task prose. Candidate routes are filtered by access, profile ceilings, and provider independence, then follow configured harness preference; auto routes refresh and fail over when the preferred provider becomes unhealthy during live revalidation, while explicit routes fail closed. Jobs retain the matched IDs and catalog revision as capability provenance; workflow fingerprints bind the task context and permit only monotonic budget increases on replay, while journal routes retain concrete harness/model/status/error and replacement linkage.
