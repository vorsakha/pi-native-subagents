...
function detailSignature(run: WorkflowSnapshot, phase: WorkflowPhase | undefined, agent: WorkflowAgentRecord | undefined): string {
  const pieces = [
    run.runId,
    run.name,
    run.description,
...
function boundRenderedRows(rows: string[], limit: number, theme: Theme, label: string): string[] {
...
function boundedTranscriptParts(entries: TranscriptEntry[], limit: number): Array<{ entry?: TranscriptEntry; text: string }> {
...
function appendBoundedSection(target: string[], theme: Theme, label: string, rows: string[], limit: number): void {
...
function renderPrefixedRows(theme: Theme, prefix: string, text: string, color: Parameters<Theme["fg"]>[0], width: number): string[] {
