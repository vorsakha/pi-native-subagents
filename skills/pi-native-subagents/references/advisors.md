...
`advisor_reset` replaces only the native identity. It keeps stable ID, policy, cumulative spend, and ledger; increments lineage; and clears continuation/generation. Never reset to bypass budget. `advisor_close` deletes the entry, aliases, and roster slot after active or queued calls settle or cancel. Roster changes publish only after storage commits. Lifecycle failure leaves the prior identity open. A failed consultation keeps one private settlement; later consult/reset flushes it, while close discards the advisor.

`/advisors` is this thread's specialist roster and inspector: immutable read-only policy, state, queue, cumulative usage against budget, lineage/generation, last consultation, and bounded ledger provenance. Ask, reset, and close are keyboard accessible, and destructive actions confirm. Selection stays visible in short terminals; completed and persisted latest answers wrap in a scrollable detail view. Private native continuation IDs and paths are never rendered. Advisor turns stay on `/advisors` and never appear as direct subagent results in the shared activity widget. Human shortcuts are `/advisor open`, `/advisor ask`, `/advisor reset`, and `/advisor close`.
