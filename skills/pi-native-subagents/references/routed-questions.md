...
- Cancelling the caller or shutting down the parent session rejects the parked tool callback. Dismissing or expiring a peer question also removes queued answer work and cancels an answer turn that already started; that target's retained session is then unavailable for later `followUp()` calls. The workflow waits for native cancellation to settle before it restores the target projection or writes the terminal peer journal, so usage reported during cancellation is retained. Late answers never revive either job.
