...
- A peer answer is a real model turn on the target lineage: it is charged to that agent's usage, rechecks that agent's cumulative retained-session budget, counts toward workflow token/cost/turn budgets, and appears as another bounded generation (`peerAnswer`) under the same agent card — never a new agent. Successful and failed answer journals include all target-lineage usage, including a continued lineage's failed primary and any failed answer-turn spend. A failed peer answer marks only its own generation failed; the lineage result the script already consumed is preserved.
