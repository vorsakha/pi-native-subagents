...
- An implementer may explicitly declare `continuationFallback` on its initial options. After eligible progressed provider unavailability, the bounded handoff includes the current round and pending findings; the same logical implementer ID remains in convergence state while later fix rounds target the replacement's retained session. The reviewer lineage is unchanged. A failed or unsafe handoff ends convergence as `failed`, never as a fresh round or provider loop.
