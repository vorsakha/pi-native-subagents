...
- Transcripts default to a compact mode that collapses consecutive tool calls into one bounded indicator with counts and running/failed visibility; full native tool rendering is a toggle. Workflow inspection preserves run IDs and phase/agent node identities through refreshed snapshots, phase insertion, sorting, filtering, reordering, replacement restart, and resize. If a node disappears, selection falls back to its containing phase, then the current phase, then the first visible node.
