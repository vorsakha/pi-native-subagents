...
- `/subagents` opens the adaptive dashboard for direct jobs. It has explicit `job-list`, `job-detail`, and `composer` focus. The composer records whether it answers, steers, or queues a follow-up. Enter or Right inspects a selected job; `a`, `s`, and `f` are the intervention verbs. Escape or Left backs out one layer. In list focus, `j/k` selects jobs. In detail focus, `j/k` scrolls the transcript. Drafts and tail-follow state survive focus and layout changes. Cancellation still requires the exact eligible job rendered by the latest view.
- `/workflows` is the durable surface for workflow runs. It uses explicit `runs`, `outline`, and `agent-detail` focus instead of implicit run/phase/agent navigation. The bounded outline always models every recorded or planned phase and expands the selected phase's filtered agents. Agent rows carry lifecycle state and semantic progress; phase rows remain visible regardless of the agent filter. Workflow agents stay controlled through `/workflows`, not `/subagents`.
...
- A selected direct job, workflow run, or workflow agent starts with one bounded state preview and its available next action before route, usage, and provenance. The transcript, structured result, and detail sections hold the full evidence; the preview does not repeat full tool calls or result bodies.
