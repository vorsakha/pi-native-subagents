...
- `/workflows` supervision uses explicit `runs` → `outline` → `agent-detail` focus. Use its contextual `?` legend, and back out one layer at a time. Agent restart and cancellation apply only to an eligible selected agent node that the outline or detail view visibly rendered.
