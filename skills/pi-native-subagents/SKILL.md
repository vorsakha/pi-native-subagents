...
This permits one bounded handoff after authoritative provider unavailability and observed progress. It keeps the logical job ID stable across replacement and follow-ups, rechecks checkout, readiness, capabilities, and budgets at queued startup, and never replays the primary or falls through to provider waiting. Do not combine it with `providerFallback` or worktree isolation. Read `references/progressed-continuation.md` first.
