...
- `/workflows` pins a live-only `Now` summary and workflow `Context` for a running agent. It uses bounded event evidence, never provider thoughts, response text, raw commands/results, credentials, or absolute paths. Questions, failures, peer answers, provider waits, and queueing outrank routine activity. Replay never restores `Now`. Use `?` for navigation and `i` for routine telemetry.
