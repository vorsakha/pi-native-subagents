...
- Live, model-free `subagent_capabilities` discovery of each harness's real native tools, skills, commands, plugins, MCP servers, and hooks, with the access ceiling already applied; `requires` and `harness: "auto"` route to a harness proven to have a capability (or a healthy/authenticated route when requirements are omitted), live-revalidated right before dispatch.
...
Behavior comes from the task plus a short isolation baseline. Give every model-spawned child a complete task containing the relevant paths, requirements, constraints, and verification expectations. Human `/subagent` jobs can additionally pull a filtered parent-thread snapshot when their task refers to the current discussion.
...
message into the orchestrator's context. Every such child receives the read-only
`parent_thread_context` tool, backed by a snapshot captured when the command was
run. The snapshot retains bounded user/assistant text and conversation summaries,
while excluding assistant thinking, tool calls/results, system/developer prompts,
and extension-only state. The child decides whether to call the tool; no parent
conversation is injected automatically. The job appears as one TUI-only card
