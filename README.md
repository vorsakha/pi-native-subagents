...
Standalone Pi package for session-scoped subagent lifecycle management across native Pi, Claude Code, and Codex backends.
...
- Automatic terminal notifications and a compact footer status without injecting completion messages into model context.
- Bounded in-memory transcript/status reducer; the package writes no custom logs or transcript copies.
...
Child-native Pi, Claude, and Codex sessions use each CLI's own private session storage. This package does not create parallel logs, transcript files, or project-local runtime state.
